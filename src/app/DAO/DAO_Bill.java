...
public class DAO_Bill {
	  public List<Bill> selectAll() {
        ...
        try (Connection con = XJdbc.getConnection();
            ...
            while (rs.next()) {
                // *** 1. Lỗi NPE được khắc phục nhờ sửa constructor của Bill ***
                Bill bill = new Bill(); 
                ...
                // *** 2. Sửa code: Vẫn gán giá trị, kể cả là NULL từ DB (cho LEFT JOIN) ***
                // bill.getCustomer() không còn là NULL nữa!
                ...
                bill.getEmployee().setName(rs.getString("employeeFullName"));   
    ...
    public List<Bill> getBillsByCustomerId(String customerId) {
        // SQL chỉ cần lọc theo customerId, không cần LEFT JOIN Employee
        String sql = "SELECT \n" +
                     "    B.billId, B.dateCreated, B.hourIn, B.hourOut, B.phoneNumber, B.total, B.custPayment, B.status, B.customerId, B.employeeId, B.tableId,\n" +
                     "    CONCAT(C.firstName, ' ', C.lastName) AS customerFullName, \n" +
                     "    NULL AS employeeFullName -- Không cần thiết cho chức năng này\n" +
                     "FROM \n" +
                     "    BILL B \n" +
                     "LEFT JOIN \n" +
                     "    Customer C ON B.customerId = C.customerId \n" +
